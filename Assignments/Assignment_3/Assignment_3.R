...
dat = read.csv("./thatch_ant.csv", na.strings = c("",NA))
names(dat)
...
#The Headwidth..mm.. the tibble is continous along the y axis; 
#Headwith though  is a factor
...
##(I did it on the read.csv() line with ) 
...
dat3<-dat%>%select(Colony,Mass)%>%slice(1:20)
...
write.csv(dat3,"./FULLMER_first_file.csv")
